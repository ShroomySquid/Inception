...
if [ ! -f /var/www/html/wp-config.php ]; then
  ...
  wp core install --allow-root --home="${DOMAIN_NAME}" --url="${DOMAIN_NAME}" --title="${WP_TITLE}" --admin_name="${WP_ADMIN}" --admin_password="${WP_PASSWORD}" --admin_email="${WP_EMAIL}" --skip-email
